...
- **The endpoints moved under the customer.** There is no separate "retail client" to register, and no `POST .../clients` step. An account holder IS the customer you created with `POST /api/v1/partner/users` — that was always true in our data model, and publishing it as a second entity gave you two identities and two KYC statuses to keep in step for one person. Submitting an identity is now `POST /api/v1/partner/users/{user_id}/identification`, and issuing is `POST /api/v1/partner/users/{user_id}/fiat-accounts`. Reads live at `/api/v1/partner/fiat-accounts`.
- **Identity comes from the verification, not from your request body.** Every field on the identification request is now optional; what we verified at KYC is preferred over anything you send. Call `GET /api/v1/partner/users/{user_id}/identity` and send only what it lists under `missing`. You are no longer asked to retype a name, a country or a document we already read off the customer's ID.
- **Polling a verification is a `GET`.** `GET /api/v1/partner/users/{user_id}/identification` replaces the old `POST .../identification/refresh`. Checking a status never changed anything; it should not have needed a write.
