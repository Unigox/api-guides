...
- **`?status=` accepts both new values**, and `completed` now excludes a delayed order that has released but not paid. One gap: `status=cancelled` does not return a delayed order whose crypto was refunded *after* the release — read `delayed_settlement_crypto_refunded_to_customer_at` on the order for that case.
