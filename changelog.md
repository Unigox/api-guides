...
For payout corridors where the provider verifies recipients (e.g. Switch mobile money and bank transfers in Nigeria, Ghana, and Kenya), `POST /api/v1/partner/offramp/initiate` now checks the recipient up front:
...
- If the verification provider is temporarily unavailable, the order is still created and the recipient is re-checked before payout — so a provider outage never blocks order creation.
