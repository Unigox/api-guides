...
**Some off-ramp quotes now come back as delayed settlement (T+1): the crypto leaves escrow on your own release signature, and the fiat reaches the recipient within a stated window afterwards.** This is a different product from the one the same endpoint returns the rest of the time — on an ordinary order the crypto leaves *after* the fiat is confirmed — so the flag has to be read on every quote rather than assumed absent. See the [Delayed settlement (T+1)](./api-reference/delayed-settlement.md) reference.

- **The quote says so.** `POST /api/v1/partner/offramp/quote` carries `delayed_settlement` (boolean, **always present**, so a client can tell "this build does not know about T+1" from "this quote is not T+1") and `settlement_hours` (integer or `null` — the payout window in whole hours, counted from the release, not from now). Initiating such a quote without showing your customer the wait sells them something else, and there is no later endpoint that reveals it.
- **You cannot request it or decline it.** An instant offer that covers the amount always wins, whatever its rate; a delayed offer is considered only when none does. It is never offered on a widget order, where the wallet is your customer's and nobody on this API can sign the release — delayed settlement exists only on orders whose crypto you hold.
- **You sign the release.** A delayed order parks at `crypto_received` with `next_action: "sign_settlement_consent"`, `confirm-fiat-received` removed from `allowed_actions` and `settlement-consent` in its place. On a held order — flagged for review, or with a failed payout whose escrow refund is in flight — `next_action` is `null`, the list is left as the ordinary mapping produced it, and both consent endpoints answer `409 OPERATION_NOT_ALLOWED`. `GET /api/v1/partner/orders/{order_id}/settlement-consent-parameters` returns the EIP-712 Safe transaction, and `POST /api/v1/partner/orders/{order_id}/settlement-consent` takes the signature. Same shape as the refund pair you already use, with `direction: "to_buyer"` and one addition: `signed_data` is **required**, and is either the returned `safe_params.data` (the release calldata) or the returned `tx_hash`. Only the artefact you signed distinguishes "send the crypto to the vendor" from "send it back to me", so there is nothing to check a signature against without it.
- **`release_started: false` on that response is a success.** At or above the threshold published as `threshold_usd` — USD 50 000 today, and an order whose USD equivalent cannot be computed counts as above it — the release waits for a source-of-funds review, and on that class of order this is the normal outcome; `next_action` then says what is still owed. Below the threshold the crypto goes immediately.
- **Large orders need a source-of-funds dossier, collected on your screens.** `GET /api/v1/partner/orders/{order_id}/source-of-funds` returns the case, `POST …/source-of-funds/declaration` records what your customer declared, and `POST …/source-of-funds/documents` (multipart) takes one evidence file each. Below the threshold all three answer `404 ORDER_NOT_FOUND`, which is an answer — nothing is owed — not a failure.
- **Render the form from `GET /api/v1/partner/orders/{order_id}/source-of-funds/requirements`.** It is the document catalogue: which categories may be declared, which documents each one owes, and how much statement history it asks for. It answers for any delayed order of yours, including one below the threshold. `?source_of_funds=` narrows it to one category; an unknown code answers `404`. You need it because until a declaration is posted the dossier carries no frozen requirement to read, and guessing the keys is how evidence gets uploaded against a type no requirement names. Once a declaration exists, `requirements_snapshot` on the dossier replaces it: that is the contract frozen when the declaration was accepted, so a rule edited while your case is open cannot change what it owes. Categories marked `legacy` are served only so an old dossier can resolve its label — the declaration endpoint refuses them, so drop them from your picker.
- **`requirements_snapshot.review_checks` is always `[]`.** It is the reviewer's own checklist — what each document is tested against — and published to an integrator it would be a specification for passing a check rather than for meeting one. The key is kept and emptied rather than dropped, so nothing mapping over it meets a second shape.
- **Two new order statuses, both unreachable for an ordinary order.** `settlement_in_progress` — the crypto has left escrow and the fiat has not arrived — and `returned` — the bank sent the payout back and no fresh authorisation stands, which is not `cancelled` because nothing has been given back to anyone and the order will be paid again. `completed` on a delayed order now means the money actually reached the customer.
- **You can cancel a parked delayed order, and only a parked delayed one.** `allowed_actions` on a parked order carries `["settlement-consent", "cancel"]`. `cancel` is the entry you would not otherwise find: this is the only funded state any order can be cancelled from — every other funded order answers "cannot be cancelled" — and `POST /api/v1/partner/orders/{order_id}/cancel` is the ordinary endpoint, refunding the escrow to you while the crypto is still in it. **Both actions are withdrawn the moment the consent is signed**, and both then answer `409 OPERATION_NOT_ALLOWED`: after the signature the crypto is on its way to the vendor, so a cancel would be offering to refund an escrow that is being released. The cancel loses that race in the database rather than in a check, so calling it against an order that is already releasing cannot half-cancel it.
- **`consent_deadline_at` says how long a parked order will wait.** It is on the order payload and on `settlement-consent-parameters` — funding time plus the consent window, after which the ordinary payment-window sweep refunds the escrow. This is the only deadline that exists *before* the release (`payout_deadline_at` is measured from the release, which has not happened yet), so it is the one to show your customer while asking them to sign. `null` on any order that is not parked, because by then it either released or it was refunded. Two omissions are deliberate and follow from the same rule — a key that could only ever be `null` is a worse contract than its absence: `settlement-consent-parameters` carries no `payout_deadline_at`, because signing is what starts that clock, and the webhook payload carries no `consent_deadline_at`, because every T+1 event fires after the release.
- **Ten fields on the order say where the money is.** `delayed_settlement`, `settlement_hours`, `consent_deadline_at`, `payout_deadline_at` and the six `delayed_settlement_*_at` marks are present on every order, `null` on an ordinary one. They exist because `status` alone cannot carry this: `completed`, `settlement_in_progress` and `returned` all live in the same internal states, and a `null` mark on a delayed order is a fact — that step has not happened — rather than an absent field.
- **`?status=` accepts both new values**, and `completed` now excludes a delayed order that has released but not paid. `cancelled` returns the orders that stopped before settlement as before, plus delayed orders whose crypto was refunded after the release — that order reports `cancelled` while still sitting in a release state internally. The four filters are disjoint and between them cover every released delayed order, so what a filter returns is exactly the set whose own `status` reports that value.
- **The list and the order page agree.** `GET /api/v1/partner/orders` computes `next_action` and `allowed_actions` exactly as `GET /api/v1/partner/orders/{order_id}` does, so a list row never asks you to sign a consent you have already given, and `settlement-consent` is withdrawn from both once it is in. This keeps the guarantee made on 2026-08-26: an action listed in `allowed_actions` is one the endpoint will accept.
- **`order.status.changed` fires at each of the six marks** — crypto sent to the payout provider, payout authorised, payout submitted by the provider, fiat paid, bank returned, crypto refunded. Without them you would hear nothing between the release and the payment, because the underlying status does not move for the whole second half of a delayed order's life. Nine of the order's T+1 fields travel on every event for that order, not only the one each mark triggered, and are **omitted entirely on an ordinary order** — `delayed_settlement` is `true` or absent, never `false`. On the webhook a mark that has not happened is absent rather than `null`, which is the opposite of the order payload. A bank return can recur (returned, re-authorised, returned again) and `event_id` is unique per emission: de-duplicate on `event_id`, never on the mark. No subscription change is needed.
- **Nothing changes for an instant order.** Its quote reports `delayed_settlement: false`, its timeline is unchanged entry for entry, and its webhook payload is byte-for-byte what it was.
...
- **The endpoints moved under the customer.** There is no separate "retail client" to register, and no `POST .../clients` step. An account holder IS the customer you created with `POST /api/v1/partner/users` — that was always true in our data model, and publishing it as a second entity gave you two identities and two KYC statuses to keep in step for one person. Submitting an identity is now `POST /api/v1/partner/users/{user_uuid}/identification`, and issuing is `POST /api/v1/partner/users/{user_uuid}/fiat-accounts`. Reads live at `/api/v1/partner/fiat-accounts`.
- **Identity comes from the verification, not from your request body.** Every field on the identification request is now optional; what we verified at KYC is preferred over anything you send. Call `GET /api/v1/partner/users/{user_uuid}/identity` and send only what it lists under `missing`. You are no longer asked to retype a name, a country or a document we already read off the customer's ID.
- **Polling a verification is a `GET`.** `GET /api/v1/partner/users/{user_uuid}/identification` replaces the old `POST .../identification/refresh`. Checking a status never changed anything; it should not have needed a write.
- **The envelope is the standard one.** `{"success": true, "data": { … }}` on success, and `{"success": false, "error": {"code", "message", "details"}}` on failure — the same `PartnerErrorResponse` the order and off-ramp endpoints use. Error codes are `SCREAMING_SNAKE` like everywhere else (`MISSING_FIELDS`, `CLIENT_NOT_APPROVED`, `PROVISIONING_IN_PROGRESS`), and a validation failure lists the fields under `error.details.missing_fields`. The old flat `{"ok", "source", "error", "detail"}` envelope is gone, along with the `source`-is-sometimes-an-object exception on the account list.
- **Field names are `snake_case`,** at every depth, matching the rest of the API. They were `camelCase`.
- **Nothing identifies our banking provider any more.** `source`, `upstream`, the `depa*` identifiers and the raw provider status are all removed, and the error codes that named the provider (`depa_error`, `depa_unavailable`) are now `BANKING_ERROR` and `BANKING_UNAVAILABLE`. Which institution issues a given currency is an operational detail we may change, and no integration should have been able to tell. `bank_name` and `bic` stay — those describe the account a payer sends money to, which is yours to display.
- **Business-held accounts are on this API for the first time, on the same endpoints.** An account you issued to a company you onboarded through KYB was previously readable only in the console, under its own `business-customers/{caseId}/accounts/{bankAccountId}` tree and a third response envelope. It now appears in `GET /api/v1/partner/fiat-accounts` beside your retail ones, in the same shape, and its ledger and payments are read through the same routes. `holder_type` (`retail` | `business`) says whose account it is and `holder_id` names them in that kind's own id space — a customer `user_uuid`, or a KYB case id. Filter with `?holder_type=`; if you were never set up for business accounts you simply have none, which is not an error. Issuing one still needs a verified KYB case and stays a console flow — these are reads.
- **Account ids are opaque and carry their holder kind:** `retail_412`, `business_87`. Pass them back verbatim and do not parse them. The two kinds are stored separately and their row numbers overlap, so the prefix is part of the id rather than a parameter beside it. The console's business listing used to publish the banking provider's own bank-account id as the account's identifier; it no longer travels.
- **Unchanged:** enablement is still Unigox-side (`retail` product, `issue_retail_accounts` capability), money movement is still not on this API, issuance is still idempotent per (customer, currency, jurisdiction), and deposits still stay on the customer's account unless we have switched collection on for you — with the same `retail.settlement.completed` webhook when it settles.
...
**And the order now tells you when that refund is owed.** The endpoints above are only useful if you know to call them, and until now nothing said so: an order stuck this way reads `cancelled`, which is also what an order reads when it never funded and when its refund was paid out months ago. The status cannot tell those apart, so two other things now do.

- **On the order.** `allowed_actions` contains `authorize-refund` while the crypto is still in the escrow, with `next_action: "authorize_refund"` and an `action_required` block naming the endpoint. It appears only when there is something to send back — a funded escrow nothing has moved out of — so it is never a dead end. `cancelled` is the one terminal status worth checking `allowed_actions` on.
- **By webhook.** A new event type, `order.refund.required`, fires at most once per order, carrying the same `action_required` block. It is separate from `order.status.changed` because there is no status change to carry it — the order has read `cancelled` since the day it stopped. No subscription change is needed; if you receive `order.status.changed` today you will receive this.

Neither appears on a widget order: your customer funded that escrow and signs for it in our app, and the refund endpoints answer `404 ORDER_NOT_FOUND` for you there, as every other action endpoint does.

**Correction to the 2026-07-13 fee entry.** It said cancelled or failed orders "refund the full amount, markup included". The amount was right; the automatic refund was not. Nothing is deducted from an order that does not settle, but once the escrow has been funded the crypto stays in it until someone signs — which, before today's endpoints, meant contacting us.
...
**You can now charge your end users a per-order markup.** Pass an optional `partner_fee_pct` on the quote and estimate requests for both ramps (`POST /onramp/estimate`, `/onramp/quote`, `/offramp/estimate`, `/offramp/quote`) — `1` means 1%. The markup is declared per order (no stored config), computed in crypto on the same base as the platform fee. For off-ramp, the markup is settled on-chain to your partner wallet when the order releases successfully; an order that does not settle is never charged the markup, and the refund returns the full funded amount with the markup in it. (Corrected 2026-09-08: this originally said cancelled or failed orders "refund the full amount", which read as if the refund happened by itself. It does not once the escrow has been funded — see the 2026-09-08 entry.) For on-ramp there's no separate transfer leg: the full buyer amount lands in your partner wallet at release, and the markup is captured by withholding it when the crypto bridges out at send-out (a crypto-anchored order is sized up beforehand so the requested amount still arrives net of the withheld fee).
