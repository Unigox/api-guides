...
- **You sign the release.** A delayed order parks at `crypto_received` with `next_action: "sign_settlement_consent"`, `confirm-fiat-received` removed from `allowed_actions` and `settlement-consent` in its place. `GET /api/v1/partner/orders/{order_id}/settlement-consent-parameters` returns the EIP-712 Safe transaction, and `POST /api/v1/partner/orders/{order_id}/settlement-consent` takes the signature. Same shape as the refund pair you already use, with `direction: "to_buyer"` and one addition: `signed_data` is **required** and is the returned `tx_hash` verbatim. Only that hash distinguishes "send the crypto to the vendor" from "send it back to me", so there is nothing to check a signature against without it.
- **`release_started: false` on that response is a success.** At or above USD 50 000 the release waits for a source-of-funds review, and on that class of order this is the normal outcome; `next_action` then says what is still owed. Below the threshold the crypto goes immediately.
